...
def admin_dashboard():
    ...
    if request.method == "POST":
        ...
        if task_name:
            filters["task_name"] = {"$regex": task_name, "$options": "i"}  # Case-insensitive regex
        if assigned_user:
            filters["username"] = assigned_user
        if status:
            filters["status"] = status

    # Fetch all tasks with the applied filters (no pagination)
    tasks = list(tasks_collection.find(filters))

    # Fetch all users for the filter dropdown
    users = list(users_collection.find())

    return render_template("admin_dashboard.html", tasks=tasks, users=users, filters=filters)
